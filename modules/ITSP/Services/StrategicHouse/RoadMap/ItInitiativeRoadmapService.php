...
namespace Modules\ITSP\Services\StrategicHouse\RoadMap;
...
class ItInitiativeRoadmapService
{
            ...
            ->with([
                ...
                'mappedProjects' => fn ($q) => $q->with([
                    'reviewPcStatusImplementations' => fn ($rq) => $rq
                        ...
                        ->orderByDesc('id'),
                    'charters' => fn ($cq) => $cq
                        ->select(['id', 'project_id', 'version_label', 'status', 'objectives', 'duration', 'owner', 'leader'])
    ...
    private function mapCharter($charter, string $projectName, ?string $projectCode): array
    {
        ...
        return [
            ...
            'status_ref'    => $charter->statusRef ? ['id' => $charter->statusRef->id, 'name' => $charter->statusRef->name] : null,
