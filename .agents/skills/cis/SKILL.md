...
│           └── modules/         # Folder modul frontend (Nama wajib match dengan Backend)
│               ├── ITSP/
│               └── ITOM/
└── routes/
...
### 2. Frontend Structure (`resources/js/Pages/modules/<ModuleName>/`)

Struktur folder frontend di bawah `resources/js/Pages/modules/` harus mencerminkan pembagian modul pada backend untuk menjaga kerapian.
...
│   ├── Index.vue             # Renders via: Inertia::render('modules/<ModuleName>/SubFeatureA/Index')
│   └── Show.vue
└── SubFeatureB/
    └── Index.vue
```
...
2.  **Copy Frontend**: Salin folder `source_project/resources/js/Pages/modules/<ModuleName>` ➡️ `target_project/resources/js/Pages/modules/<ModuleName>`.
