                    ...
                    <table class="w-full min-w-[980px] divide-y divide-slate-200 text-sm dark:divide-white/10">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                            ...
                            >
                                ...
                                <td class="px-4 py-4 text-sm text-slate-800 dark:text-slate-100">
                                    {{ item.project_name || '-' }}
                                </td>
...
const codeOptions = computed(() => uniquePreserveOrder(rows.value.map((item) => item?.project_code)));
const projectNameOptions = computed(() => uniquePreserveOrder(rows.value.map((item) => item?.project_name)));
...
const filteredRows = computed(() => {
    return rows.value.filter((item) => {
        if (selectedCode.value !== 'all' && String(item?.project_code ?? '') !== selectedCode.value) {
            return false;
        }

        if (selectedProjectName.value !== 'all' && String(item?.project_name ?? '') !== selectedProjectName.value) {
            return false;
        }

        if (selectedType.value !== 'all' && String(item?.project_type ?? '') !== selectedType.value) {
            return false;
        }

        if (selectedStatus.value !== 'all' && String(item?.status_id ?? '') !== selectedStatus.value) {
            return false;
        }
