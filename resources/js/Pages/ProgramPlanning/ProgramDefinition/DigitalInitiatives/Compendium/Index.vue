                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            Data gabungan dari <code>mst_initiative</code> dan <code>trs_sc_initiative</code>.
                        </p>
                    </div>
                ...
                <table class="w-full table-fixed divide-y divide-slate-200 text-[11px] dark:divide-white/5">
                    ...
                    <thead class="bg-slate-50 dark:bg-white/5">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Group</th>
                            <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">No</th>
                            ...
                            <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">DataSource</th>
                            <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Waktu DataSource Created</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white dark:divide-white/5 dark:bg-[#1a1a1a]">
                        <tr v-for="(item, index) in compendiumItems" :key="`compendium-${item.id}`" class="transition-colors hover:bg-slate-50 dark:hover:bg-white/5">
                            ...
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.group ?? '-' }}</td>
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.no ?? '-' }}</td>
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.project_owner ?? '-' }}</td>
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.use_case ?? '-' }}</td>
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.desc ?? '-' }}</td>
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.value ?? '-' }}</td>
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.urgency ?? '-' }}</td>
                            ...
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.data_source ?? '-' }}</td>
                            <td class="px-3 py-3 text-slate-700 dark:text-slate-200">{{ item.data_source_created ?? '-' }}</td>
                        ...
                        <tr v-if="compendiumItems.length === 0">
                            <td colspan="13" class="px-6 py-10 text-center text-xs text-slate-500 dark:text-slate-400">
                                Belum ada data compendium dari <code>mst_initiative</code> dan <code>trs_sc_initiative</code>.
                            </td>
                        </tr>
                    </tbody>
                </table>
