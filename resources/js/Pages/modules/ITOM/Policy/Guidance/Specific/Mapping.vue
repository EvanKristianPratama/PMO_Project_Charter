            ...
            <section class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#171717]">
                ...
                <div class="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <Link
                            :href="route('itom.policy.general.index', { regulation_id: props.selectedRegulationId || activeRegulation?.id })"
                        ...
                        <Link
                            :href="route('itom.policy.specific.manage', { regulation_id: props.selectedRegulationId || activeRegulation?.id })"
            ...
            <div class="space-y-8">
                ...
                <div v-if="objectives.length > 0" class="flex justify-end mb-4">
                    <div class="inline-flex rounded-lg bg-slate-100 p-1 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                        ...
                        <Link 
                            :href="route('itom.policy.specific.mapping.analysis', { regulation_id: selectedRegulationId })" 
...
function submitMappingForm() {
    if (editingMappingId.value) {
        mappingForm.put(route('itom.policy.cobit-mapping.update', editingMappingId.value), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => cancelMappingForm(),
        });
    } else {
        mappingForm.post(route('itom.policy.cobit-mapping.store'), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => cancelMappingForm(),
        });
...
function deleteMapping(map) {
    if (confirm('Yakin ingin menghapus mapping ini?')) {
        router.delete(route('itom.policy.cobit-mapping.destroy', map.id), {
            preserveScroll: true,
        });
    }
}
...
function submitUpdateObjective(id) {
    editObjectiveForm.put(route('itom.policy.objective.update', { objective: id }), {
        preserveScroll: true,
        onSuccess: () => {
            editingObjectiveId.value = null;
