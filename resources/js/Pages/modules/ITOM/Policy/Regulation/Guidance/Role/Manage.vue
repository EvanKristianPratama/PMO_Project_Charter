            ...
            <section class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#171717]">
                ...
                <div class="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <Link
                            :href="route('itom.policy.roles.index')"
...
function toggleRoleMappingForResp(resp, role) {
    ...
    if (alreadyMapped) {
        // Remove mapping
        router.delete(route('itom.policy.roles.mapped-responsible.destroy', { roleId: role.id, responsibleId: resp.id }), {
            preserveScroll: true,
            onSuccess: () => {
        ...
        // Add mapping
        router.post(route('itom.policy.roles.mapped-responsible.store'), {
            role_id: role.id,
...
function submitCreateRole() {
    roleForm.post(route('itom.policy.roles.role.store'), {
        onSuccess: () => {
            isCreatingRole.value = false;
            roleForm.reset();
...
function submitUpdateRole(id) {
    editRoleForm.put(route('itom.policy.roles.role.update', id), {
        preserveScroll: true,
        onSuccess: () => {
            editingRoleId.value = null;
            editRoleForm.reset();
...
function deleteRole(role) {
    if (confirm(`Apakah Anda yakin ingin menghapus Role "${role.name}" beserta seluruh daftar Tanggung Jawab di dalamnya?`)) {
        router.delete(route('itom.policy.roles.role.destroy', role.id), {
            preserveScroll: true,
            onSuccess: () => {
...
function submitResponsibilityForm() {
    if (editingRespId.value) {
        // Update mode
        respForm.put(route('itom.policy.roles.responsibility.update', editingRespId.value), {
            preserveScroll: true,
            onSuccess: () => {
                activeRespRoleId.value = null;
                editingRespId.value = null;
        ...
        // Create mode
        respForm.post(route('itom.policy.roles.responsibility.store'), {
            preserveScroll: true,
            onSuccess: () => {
                activeRespRoleId.value = null;
                respForm.reset();
...
function deleteResponsibility(resp) {
    if (confirm('Apakah Anda yakin ingin menghapus tanggung jawab ini?')) {
        router.delete(route('itom.policy.roles.responsibility.destroy', resp.id), {
            preserveScroll: true,
            onSuccess: () => {
...
function submitMappingForm() {
    ...
    mappingForm.post(route('itom.policy.roles.mapped-responsible.store'), {
        preserveScroll: true,
        onSuccess: () => {
            activeMappingRoleId.value = null;
            mappingSearchQuery.value = '';
...
function deleteMapping(roleId, responsibleId) {
    if (confirm('Apakah Anda yakin ingin menghapus pemetaan master responsible ini?')) {
        router.delete(route('itom.policy.roles.mapped-responsible.destroy', { roleId, responsibleId }), {
            preserveScroll: true,
            onSuccess: () => {
