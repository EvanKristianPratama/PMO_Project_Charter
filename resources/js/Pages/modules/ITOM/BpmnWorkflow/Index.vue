...
const BpmnModeler = defineAsyncComponent(() => import('@/Components/modules/ITOM/Bpmn/BpmnModeler.vue'));
...
const saveWorkflowToDb = async () => {
    ...
    router.post(route('itom.bpmn-workflow.store'), {
...
const syncFromSop = async () => {
    ...
    try {
        const response = await axios.post(route('itom.bpmn-workflow.sync-from-sop', id));
    ...
    if (isTask) {
        ...
        if (actionType !== 'none') {
            ...
            if (liveExecution.value) {
                ...
                axios.post(route('itom.bpmn-workflow.trigger-action'), {
