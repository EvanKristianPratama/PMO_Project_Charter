<template>
    <ModulLayout title="RACI Analisis">
        <div class="space-y-4 animate-fade-in-up -mt-3 print:m-0 print:p-0">
            <Deferred :data="['regulations']">
                <template #fallback>
                    <div class="h-8 w-64 bg-slate-200 dark:bg-white/5 rounded-lg animate-pulse -mt-5"></div>
                </template>
                <PolicyTabs :regulations="regulations" />
            </Deferred>
...
const displayedPracticeIds = computed(() => {
    const ids = [];
    if (!filteredObjectives.value) return ids;
...
const raciLookup = computed(() => {
    if (!props.mappings) return new Map();
