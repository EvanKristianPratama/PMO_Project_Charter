...
import CharterCompare from '@/Components/modules/ITSP/ITInitiative/CharterCompare.vue';
import ReviewContent from '@/Components/modules/ITSP/ReviewPC/ReviewContent.vue';
...
watch(selectedProjectId, (nextValue, previousValue) => {
    ...
    router.get(route('itsp.it-initiatives.show', routeParams), {}, { preserveScroll: true });
});
...
const addRoadmap = () => {
    if (!resolvedRoadmapPcId.value) return;
    router.get(route('itsp.roadmap.edit', {
...
const saveCharter = () => {
    if (isNewVersion.value) {
        ...
        form.post(route('itsp.it-initiatives.charter.store', props.itInitiative.id), {
            preserveScroll: true,
            onSuccess: () => {
        ...
        form.put(route('itsp.it-initiatives.charter.update', [props.itInitiative.id, charterId]), {
            preserveScroll: true,
            onSuccess: () => {
                isEditing.value = false;
                isNewVersion.value = false;
...
const saveVersionAnalysis = () => {
    const targetRoute = selectedVersionAnalysisId.value
        ? route('itsp.it-initiatives.version-analysis.update', [props.itInitiative.id, selectedVersionAnalysisId.value])
        : route('itsp.it-initiatives.version-analysis.store', props.itInitiative.id);
