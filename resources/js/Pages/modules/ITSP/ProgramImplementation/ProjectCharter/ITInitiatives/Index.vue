...
import MasterInitiativeTable from "@/Components/modules/ITSP/ITInitiative/MasterInitiativeTable.vue";
import StatusImplementationTable from "@/Components/modules/ITSP/ITInitiative/StatusImplementationTable.vue";
...
import ValueCreation from "@/Components/modules/ITSP/ProgramImplementation/ValueCreation/ValueCreation.vue";
import ResourceManagement from "@/Components/modules/ITSP/ProgramImplementation/ResourceManagement/ResourceManagement.vue";
...
const showAllProjectCharter = () => {
    if (props.tableMode === "value_creation") {
        router.get(route("itsp.it-initiatives.index"), { tableMode: "flow" });
    } else {
...
const showMasterItInitiatives = () => {
    if (props.tableMode === "value_creation") {
        router.get(route("itsp.it-initiatives.index"), { tableMode: "master" });
    } else {
        hasTableSelection.value = true;
...
const showRoadmapView = () => {
    if (props.tableMode === "value_creation") {
        router.get(route("itsp.it-initiatives.index"), { tableMode: "roadmap" });
    } else {
        hasTableSelection.value = true;
...
const showImplementationView = () => {
    if (props.tableMode === "value_creation") {
        router.get(route("itsp.it-initiatives.index"), {
...
const showValueCreationView = () => {
    router.get(route("itsp.it-initiatives.value-creation"));
};

const showResourceManagementView = () => {
    router.get(route("itsp.program-implementation.resources-management.index"));
};
...
const addRoadmapHref = computed(() => {
    ...
    return selectedPcId > 0
        ? route("itsp.roadmap.add", { pc_id: selectedPcId })
        : route("itsp.roadmap.add");
});
