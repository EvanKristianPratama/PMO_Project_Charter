<template>
    <UserLayout title="Edit IT Initiative">
        <div class="mx-auto max-w-[1860px] animate-fade-in space-y-6">
            <div>
                <Link
                    :href="route('itsp.it-initiatives.index')"
                                ...
                                <td class="px-3 py-3 text-[10px] font-medium align-top">
                                    <div class="flex flex-col items-start gap-1">
                                        <Link
                                            ...
                                            :href="route('itsp.it-initiatives.show', { project: props.itInitiative.id, tab: 'detail' })"
...
const submitProject = () => {
    form.put(route('itsp.it-initiatives.update', props.itInitiative.id), {
        preserveScroll: true,
    });
};
...
const updateProjectStatusHistory = (id) => {
    ...
    router.put(route('itsp.it-initiatives.project-status-history.update', [props.itInitiative.id, id]), {
...
const confirmDeleteHistory = () => {
    ...
    router.delete(route('itsp.it-initiatives.project-status-history.destroy', [props.itInitiative.id, pendingDeleteId.value]), {
        preserveScroll: true,
        onFinish: () => {
            closeDeleteModal();
        },
