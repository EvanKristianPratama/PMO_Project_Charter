...
const submitNote = (options = {}) => {
    ...
    if (editingNoteId.value) {
        noteForm.put(route('itsp.program-evaluation.summary-review.notes.update', editingNoteId.value), requestOptions);
        return;
    }

    noteForm.post(route('itsp.program-evaluation.summary-review.notes.store'), requestOptions);
};
...
const deleteNote = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('itsp.program-evaluation.summary-review.notes.destroy', id), {
                onSuccess: () => {
                    Swal.fire({
                        title: 'Terhapus!',
...
const selectedInitiativeId = computed({
    ...
    set: (value) => {
        ...
        router.visit(route('itsp.program-planning.program-definition.digital-initiatives.summary.index', selectedValue));
    },
});
