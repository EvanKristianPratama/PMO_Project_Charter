...
const selectedAppendixId = computed({
    ...
    set: (value) => {
        ...
        if (selectedValue === APPENDIX_CREATE_VALUE) {
            router.visit(route('itsp.program-planning.program-definition.digital-initiatives.appendix.create'));
            return;
        }
        if (selectedValue === String(appendixId.value)) return;
        router.visit(route('itsp.program-planning.program-definition.digital-initiatives.appendix.edit', selectedValue));
    },
});
...
const submit = () => {
    ...
    form.put(route('itsp.program-planning.program-definition.digital-initiatives.appendix.update', props.appendix.id), {
        preserveScroll: true,
        onSuccess: () => {
            isEditing.value = false;
        },
