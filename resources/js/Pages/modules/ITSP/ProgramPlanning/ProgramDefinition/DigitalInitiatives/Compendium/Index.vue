                        ...
                        <tbody class="divide-y divide-slate-200 bg-white dark:divide-white/5 dark:bg-[#1a1a1a]">
                            <tr v-for="(item, index) in filteredCompendiumItems" :key="`compendium-${item.id}`" class="transition-colors hover:bg-slate-50 dark:hover:bg-white/5">
                                ...
                                <td class="px-3 py-3 text-[10px] font-medium">
                                    <div class="flex items-center gap-2">
                                        <Link
                                            :href="route('itsp.program-planning.program-definition.digital-initiatives.compendium.edit', item.id)"
...
const submitCreate = () => {
    ...
    })).post(route('itsp.program-planning.program-definition.digital-initiatives.compendium.store'), {
        preserveScroll: true,
        onSuccess: () => {
            isCreateModalOpen.value = false;
            resetForm();
