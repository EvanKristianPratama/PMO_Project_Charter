...
import AppendixCharterModal from '@/Components/modules/ITSP/Appendix/AppendixCharterModal.vue';
...
const selectedCompendiumId = computed({
    ...
    set: (value) => {
        ...
        if (selectedValue === COMPENDIUM_CREATE_VALUE) {
            ...
            router.visit(route('itsp.program-planning.program-definition.digital-initiatives.compendium.create'));
            return;
        }
        if (isExisting.value && selectedValue === String(compendiumId.value)) return;
        router.visit(route('itsp.program-planning.program-definition.digital-initiatives.compendium.edit', selectedValue));
    },
});
...
const submit = () => {
    ...
    if (isExisting.value) {
        form.put(route('itsp.program-planning.program-definition.digital-initiatives.compendium.update', compendiumId.value), {
            preserveScroll: true,
        });
        return;
    }
    form.post(route('itsp.program-planning.program-definition.digital-initiatives.compendium.store'), {
        preserveScroll: true,
    });
};
...
const submitAppendix = () => {
    ...
    appendixForm.put(route('itsp.program-planning.program-definition.digital-initiatives.appendix.update', appendixId.value), {
        preserveScroll: true,
        onSuccess: () => {
            isAppendixEditing.value = false;
        },
