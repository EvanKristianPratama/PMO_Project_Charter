    ...
    <UserLayout :title="pageTitle">
        <div class="space-y-5">
            <div class="flex w-fit flex-wrap items-center gap-1.5 rounded-xl border border-slate-200 bg-white p-1 shadow-sm dark:border-white/10 dark:bg-[#171717]">
                <Link
                    :href="route('itsp.program-planning.program-definition.digital-initiatives.master.index')"
                ...
                <Link
                    :href="route('itsp.program-planning.program-definition.digital-initiatives.compendium.index')"
                    class="rounded-lg px-4 py-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-500 transition-all hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-white/5"
                >
                    Compendium List
                </Link>
                <Link
                    :href="route('itsp.program-planning.program-definition.digital-initiatives.appendix.index')"
                    class="rounded-lg px-4 py-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-500 transition-all hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-white/5"
                >
                    Appendix List
                </Link>
                <Link
                    :href="route('itsp.program-planning.program-definition.digital-initiatives.mapping.index')"
                    class="rounded-lg px-4 py-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-500 transition-all hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-white/5"
                >
                    Mapping
                </Link>
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#171717]">
                <div class="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <Link
                            :href="route('itsp.program-planning.program-definition.digital-initiatives.roadmap.index')"
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <Link
                            v-if="isEditMode"
                            :href="route('itsp.program-planning.program-definition.digital-initiatives.roadmap.create')"
                        ...
                        <Link
                            v-else
                            :href="route('itsp.program-planning.program-definition.digital-initiatives.roadmap.edit')"
                            class="inline-flex items-center rounded-lg bg-[#0B2A8A] px-3 py-2 text-xs font-semibold text-white transition hover:bg-[#102f95]"
                        >
                            Buka Proses Edit
                        </Link>
...
const handleEditInitiativeChange = () => {
    ...
    router.get(route('itsp.program-planning.program-definition.digital-initiatives.roadmap.edit'), {
...
const submitForm = () => {
    ...
    if (isEditMode.value) {
        ...
        form.put(route('itsp.program-planning.program-definition.digital-initiatives.roadmap.update', selectedMilestone.value.id), {
            preserveScroll: true,
        });
        return;
    }

    form.post(route('itsp.program-planning.program-definition.digital-initiatives.roadmap.store'), {
        preserveScroll: true,
        onSuccess: () => {
...
const confirmDelete = () => {
    ...
    router.delete(route('itsp.program-planning.program-definition.digital-initiatives.roadmap.destroy', selectedMilestone.value.id), {
        preserveScroll: true,
        onFinish: () => {
