<template>
    <component
        ...
        v-bind="embedded ? {} : { title: 'Kamus Component COBIT 2019' }"
    >
        <div class="space-y-6 animate-fade-in-up">
            <!-- Header section matching reference cbioo -->
            <div
                class="flex flex-col sm:flex-row justify-between sm:items-center bg-white dark:bg-zinc-900 p-4 border border-slate-200 dark:border-zinc-800 gap-4"
            >
                <div class="flex flex-wrap items-center gap-3">
                    <h1
                        class="text-xl font-bold text-slate-800 dark:text-zinc-100 flex items-center gap-2"
                    >
            ...
            <!-- Mode Selector -->
            <div
                class="bg-white dark:bg-zinc-900 p-3 border border-slate-200 dark:border-zinc-800"
            >
                ...
                <div
                    class="mt-3 flex justify-between items-center border-t border-slate-150 dark:border-zinc-800 pt-3"
                >
                    <div
            ...
            <div
                v-if="loading"
                class="flex flex-col items-center justify-center p-12 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800"
            >
                <ArrowPathIcon class="h-10 w-10 text-[#0f2b5c] animate-spin" />
                <p
                    class="mt-4 text-xs font-semibold text-slate-600 dark:text-zinc-400"
                >
                    Memuat data referensi COBIT dari public API...
                </p>
            </div>
            ...
            <div
                v-else-if="error"
                class="p-4 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/40 text-rose-700 dark:text-rose-400 text-xs font-medium"
            >
                <p>⚠️ Terjadi kesalahan: {{ error }}</p>
                <button
                    @click="retryLoad"
                    class="mt-2 text-rose-600 dark:text-rose-300 underline font-bold"
                >
                    Coba Lagi
                </button>
            </div>
            ...
            <div v-else>
                ...
                <div v-if="activeMode === 'gamo'" class="space-y-4">
                    <!-- Domain Tab Group -->
                    <div
                        class="bg-white dark:bg-zinc-900 p-2 border border-slate-200 dark:border-zinc-800"
                    >
                        <div
                            class="flex flex-wrap gap-1 border-b border-slate-100 dark:border-zinc-800 pb-2"
                        >
                            <button
                        ...
                        <div class="flex flex-wrap gap-1.5 mt-3">
                            <button
                                ...
                                :class="
                                    activeObjectiveId === obj.objective_id
                    ...
                    <div
                        class="bg-[#0f2b5c] text-white p-3.5 font-bold text-sm tracking-wide shadow-sm flex items-center justify-between"
                    >
                        <span
                            >{{ activeObjectiveId }} —
                            {{ cleanText(activeObjectiveName) }}</span
                        >
                    </div>
                    ...
                    >
                        <div
                            class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-1"
                        >
                            <button
                    ...
                    <div
                        class="bg-white dark:bg-zinc-900 p-4 border border-slate-200 dark:border-zinc-800 shadow-sm"
                    >
                        <!-- OVERVIEW RENDERER -->
                        <div
                            v-if="activeComponent === 'overview'"
                            class="space-y-4"
                        >
                            <div v-if="overviewData" class="space-y-4">
                                <!-- Objective Maroon Header Banner -->
                                <div
                                    class="flex flex-col md:flex-row items-stretch border-b border-slate-200 dark:border-zinc-700"
                                >
                                    <div
                                        class="flex-1 bg-[#7a2433] text-white p-4 flex flex-col justify-center"
                                    >
                                        <div
                                            class="text-[14px] font-bold tracking-wide"
                                        >
                                            Domain:
                                            {{ cleanText(activeDomainName) }}
                                        </div>
                                        <div class="mt-1 text-sm font-bold">
                                            {{ activeObjectiveId }} —
                                            {{ cleanText(activeObjectiveName) }}
                                        </div>
                                    </div>
                                    <div
                                        class="md:w-80 bg-[#7a2433] text-white flex items-center justify-center p-3 border-t md:border-t-0 md:border-l-4 border-white"
                                    >
                                        <div
                                            class="text-center font-bold text-xs"
                                        >
                                            {{
                                                activeFocusArea?.name ||
                                                "COBIT Core Model"
                                            }}
                                        </div>
                                    </div>
                                ...
                                <div class="grid grid-cols-1 gap-4">
                                    <div
                                        class="bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 p-3.5 rounded-none"
                                    >
                                        <h3
                                            class="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-zinc-100 mb-2"
                                        >
                                            Description
                                        </h3>
                                        <p
                                            class="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed"
                                        >
                                            {{
                                                cleanText(
                                    ...
                                    >
                                        ...
                                        >
                                            {{
                                                cleanText(overviewData.purpose)
                                            }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div
                                    class="grid grid-cols-1 md:grid-cols-11 items-stretch gap-2 mt-4"
                                >
                                    <!-- Enterprise Goals Box -->
                                    <div
                                        class="md:col-span-5 border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900"
                                    >
                                        <div
                                            class="bg-slate-100 dark:bg-zinc-800 px-3 py-2 border-b border-slate-200 dark:border-zinc-700 text-xs font-bold"
                                        >
                                            Enterprise Goals
                                        </div>
                                        <div class="p-3 space-y-2.5">
                                            <div
                                                v-for="eg in goalsData?.entergoals"
                                                :key="eg.entergoals_id"
                                                class="text-xs"
                                            >
                                                <span
                                                    class="font-bold text-[#0f2b5c] dark:text-sky-400"
                                                    >{{
                                                        eg.entergoals_id
                                                    }}:</span
                                                >
                                                {{ cleanText(eg.description) }}
                                            </div>
                                            <div
                                                v-if="
                                                    !goalsData?.entergoals
                                                        ?.length
                                                "
                                                class="text-xs text-slate-400 italic"
                                            >
                                                No enterprise goals mapped
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- Arrow separator -->
                                    <div
                                        class="md:col-span-1 flex items-center justify-center py-2 md:py-0"
                                    >
                                        <div
                                            class="h-8 w-8 rounded bg-slate-100 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 flex items-center justify-center font-bold text-slate-700 dark:text-zinc-300"
                                        >
                                            →
                                        </div>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <div class="p-3 space-y-2.5">
                                            <div
                                                v-for="ag in goalsData?.aligngoals"
                                                :key="ag.aligngoals_id"
                                                class="text-xs"
                                            >
                                                <span
                                                    class="font-bold text-[#0f2b5c] dark:text-sky-400"
                                                ...
                                                >
                                                {{ cleanText(ag.description) }}
                                            </div>
                                            <div
                                                v-if="
                                                    !goalsData?.aligngoals
                                                        ?.length
                                                "
                                                class="text-xs text-slate-400 italic"
                                            >
                                                No alignment goals mapped
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div class="p-3 space-y-4">
                                            ...
                                            >
                                                <div
                                                    class="text-xs font-bold text-[#0f2b5c] dark:text-sky-400 border-b border-slate-100 dark:border-zinc-800 pb-0.5"
                                                >
                                                    {{ eg.entergoals_id }}
                                                </div>
                                                <ul
                                                    class="list-disc pl-4 space-y-1 text-slate-655 dark:text-zinc-350 text-[11px]"
                                                >
                                                    <li
                                                        v-for="m in eg.metrics"
                                                        :key="
                                                            m.entergoalsmetr_id
                                                        "
                                                    >
                                    ...
                                    <!-- Spacer/Separator -->
                                    <div
                                        class="md:col-span-1 py-2 md:py-0"
                                    ></div>
                        ...
                        <!-- PRACTICES RENDERER -->
                        <div
                            v-else-if="activeComponent === 'practices'"
                            class="space-y-6"
                        >
                            <!-- Summary accordion per reference show.blade.php -->
                            <div
                                class="border border-slate-200 dark:border-zinc-800"
                            >
                                <button
                                    @click="
                                        isPracticeSummaryExpanded =
                                            !isPracticeSummaryExpanded
                                    "
                                ...
                                <div
                                    v-show="isPracticeSummaryExpanded"
                                    class="p-3 border-t border-slate-200 dark:border-zinc-800 overflow-x-auto"
                                >
                                    <table
                                        class="w-full border-collapse border border-slate-200 dark:border-zinc-700 text-xs"
                                    >
                                        <thead
                                            class="bg-[#0f2b5c] text-white text-center font-bold"
                                        >
                                            <tr>
                                                <th
                                                    rowspan="2"
                                                    class="p-2 border border-slate-200 text-left min-w-[200px]"
                                                >
                                                    Practice
                                                </th>
                                                <th
                                                ...
                                                <th
                                                    rowspan="2"
                                                    class="p-2 border border-slate-200 w-[80px]"
                                                >
                                                    Total
                                                </th>
                                            </tr>
                                            <tr>
                                                <th
                                                    class="p-1 border border-slate-200 w-[70px]"
                                                >
                                                    Lv 2
                                                </th>
                                                <th
                                        ...
                                        <tbody>
                                            <tr
                                                v-for="ps in practicesSummaryList"
                                                :key="ps.practice_id"
                                                class="hover:bg-slate-50 dark:hover:bg-zinc-850"
                                            >
                                                <td
                                                    class="p-2 border border-slate-200 font-bold text-slate-700 dark:text-zinc-300"
                                                >
                                                    {{ ps.practice_id }} —
                                                    {{
                                                        cleanText(
                                                ...
                                                <td
                                                    class="p-2 border border-slate-200 text-center font-semibold"
                                                >
                                                    {{ ps.total || "-" }}
                                                </td>
                                            </tr>
                                            <tr
                                                class="bg-amber-50 dark:bg-zinc-800 font-bold border-t-2 border-slate-300 dark:border-zinc-650"
                                            >
                                                <td
                                                    class="p-2 border border-slate-200 text-right"
                                                >
                                                    Total
                                                </td>
                                                <td
                            ...
                            <!-- List of practices -->
                            <div
                                v-for="p in activeObjectivePractices"
                                :key="p.practice_id"
                                class="border border-slate-200 dark:border-zinc-800"
                            >
                                <!-- Process Component Banner -->
                                <div
                                    class="bg-[#1a3665] text-white px-3 py-2 text-xs font-bold"
                                >
                                    A. Component: Process
                                </div>
                                ...
                                <div
                                    class="grid grid-cols-1 md:grid-cols-2 border-b border-slate-200 dark:border-zinc-800 text-xs"
                                >
                                    <div
                                        class="p-3 border-b md:border-b-0 md:border-r border-slate-200 dark:border-zinc-800"
                                    >
                                        <div
                                            class="font-bold text-[#0f2b5c] dark:text-sky-400 mb-1"
                                        >
                                            {{ p.practice_id }}
                                            {{ cleanText(p.practice_name) }}
                                        </div>
                                        <div
                                            class="text-slate-600 dark:text-zinc-300 leading-relaxed"
                                        >
                                            {{
                                                cleanText(
                                    ...
                                    <div
                                        class="p-3 bg-slate-50 dark:bg-zinc-800/50"
                                    >
                                        <div
                                            class="font-bold text-slate-700 dark:text-zinc-200 mb-1.5 uppercase tracking-wide text-[10px]"
                                        >
                                            Example Metrics
                                        </div>
                                        <ul
                                            class="list-disc pl-4 space-y-1.5 text-slate-655 dark:text-zinc-350"
                                        >
                                            <li
                                                v-for="(m, idx) in p.metrics"
                                                :key="m.id"
                                            >
                                                {{
                                                    String.fromCharCode(
                                ...
                                <!-- Activities Table -->
                                <div
                                    class="bg-slate-100 dark:bg-zinc-800/80 px-3 py-1.5 font-bold text-xs border-b border-slate-200 dark:border-zinc-800 text-slate-750 dark:text-zinc-200"
                                >
                                    Activities
                                </div>
                                <div class="overflow-x-auto">
                                    ...
                                    >
                                        <thead
                                            class="bg-slate-50 dark:bg-zinc-850 text-slate-700 dark:text-zinc-300 border-b border-slate-200 dark:border-zinc-800"
                                        >
                                            <tr>
                                                <th
                                                    class="p-2 border-r border-slate-200 dark:border-zinc-800 text-center w-[6%]"
                                                >
                                                    NO
                                                </th>
                                                <th
                                                    class="p-2 border-r border-slate-200 dark:border-zinc-800 text-left"
                                                >
                                                    Activity
                                                </th>
                                                <th
                                                    class="p-2 text-center w-[18%]"
                                                >
                                                    Capability Level
                                                </th>
                                            </tr>
                                        ...
                                        <tbody>
                                            <tr
                                                ...
                                                :key="act.activity_id"
                                                class="border-b border-slate-100 dark:border-zinc-800/60 hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                            >
                                                <td
                                                    class="p-2 border-r border-slate-200 dark:border-zinc-850 text-center font-medium"
                                                >
                                                    {{ idx + 1 }}
                                                </td>
                                                <td
                                                    class="p-2 border-r border-slate-200 dark:border-zinc-850 leading-relaxed"
                                                >
                                                    {{
                                                        cleanText(
                                                            act.description,
                                ...
                                <!-- Related Guidance -->
                                <div
                                    class="bg-slate-250 dark:bg-zinc-800 px-3 py-1.5 font-bold text-xs border-y border-slate-200 dark:border-zinc-800 flex justify-between"
                                >
                                    <span class="w-[65%]"
                                    ...
                                    >
                                    <span
                                        class="w-[35%] text-center border-l border-slate-200 dark:border-zinc-700"
                                        >Detailed Reference</span
                                    >
                                </div>
                                <div class="overflow-x-auto text-xs">
                                    <table class="w-full border-collapse">
                                        <tbody>
                                            <tr
                                                v-for="gd in p.guidances"
                                                :key="gd.guidance_id"
                                                class="border-b border-slate-100 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-850"
                                            >
                                                <td
                                                    class="p-2 w-[65%] leading-relaxed border-r border-slate-200 dark:border-zinc-800"
                                                >
                                                    {{ cleanText(gd.guidance) }}
                                                </td>
                                                <td
                                                    class="p-2 w-[35%] leading-relaxed text-slate-500 dark:text-zinc-400"
                                                >
                                                    {{
                                                        cleanText(gd.reference)
                                                    }}
                                                </td>
                                            </tr>
                                            <tr v-if="!p.guidances?.length">
                                                <td
                                                    colspan="2"
                                                    class="p-3 text-center text-slate-400 italic bg-white dark:bg-zinc-900"
                                                >
                        ...
                        <div
                            v-else-if="activeComponent === 'infoflows'"
                            class="space-y-4"
                        >
                            <div class="overflow-x-auto">
                                <table
                                ...
                                >
                                    <thead
                                        class="bg-[#0f2b5c] text-white font-bold text-center"
                                    >
                                        <tr>
                                            <th
                                                class="p-2 border border-slate-200 text-left min-w-[200px]"
                                            ...
                                            <th
                                                class="p-2 border border-slate-200 w-[120px]"
                                            >
                                                From
                                            </th>
                                            <th
                                            ...
                                            <th
                                                class="p-2 border border-slate-200 w-[140px]"
                                            >
                                                To
                                            </th>
                                        </tr>
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="(
                                                row, idx
                                            ) in activeObjectiveInfoflowRows"
                                            :key="idx"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                        >
                                            <!-- Rowspanned Practice column -->
                                            <td
                                                v-if="
                                                    activeObjectiveInfoflowRowspans[
                                                        idx
                                                    ] > 0
                                                "
                                                :rowspan="
                                            ...
                                            <td
                                                class="p-2 border border-slate-200 leading-relaxed"
                                            >
                                                {{
                                                    cleanText(row.input) ||
                        ...
                        <div
                            v-else-if="activeComponent === 'organizational'"
                            class="space-y-4"
                        >
                            <div
                                class="bg-slate-50 dark:bg-zinc-800 p-2.5 font-bold text-xs text-slate-800 dark:text-zinc-200 border-b border-slate-200 dark:border-zinc-700"
                            >
                                B. Component: Organizational Structures for
                                {{ activeObjectiveId }} —
                                {{ cleanText(activeObjectiveName) }}
                            </div>
                            <div class="overflow-x-auto">
                                ...
                                >
                                    <thead
                                        class="bg-[#d2e2d8] dark:bg-zinc-800 text-slate-800 dark:text-zinc-200"
                                    >
                                        <tr>
                                            <th
                                                class="p-2 border border-slate-200 text-left min-w-[260px] align-middle"
                                            >
                                            ...
                                            <th
                                                v-for="rn in activeObjectiveOrganizational.roles"
                                                :key="rn"
                                                class="p-0 border border-slate-200 w-[44px] text-center align-middle"
                                            >
                                                <div
                                                    class="vertical-text text-[10px] font-bold py-1 select-none text-slate-800 dark:text-zinc-355 uppercase"
                                                >
                                                    {{ cleanText(rn) }}
                                                </div>
                                            </th>
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="p in activeObjectiveOrganizational.practices"
                                            :key="p.practice_id"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                                                class="p-2 border border-slate-200 font-bold text-slate-800 dark:text-zinc-300 leading-normal truncate max-w-[400px]"
                                                :title="
                                                    p.practice_id +
                                                    ' ' +
                                                    p.practice_name
                                            ...
                                            <td
                                                v-for="rn in activeObjectiveOrganizational.roles"
                                                :key="rn"
                                                class="p-2 border border-slate-200 text-center font-extrabold text-sm"
                                                :class="
                                                    getRaciColorClass(
                                        ...
                                        <tr>
                                            <td
                                                :colspan="
                                                    activeObjectiveOrganizational
                                                        .roles.length + 1
                                                "
                                                class="p-0 border-none pt-4"
                                            >
                                                <table
                                                    class="w-full border-collapse border border-slate-200 dark:border-zinc-850 text-xs"
                                                >
                                                    <thead>
                                                        <tr
                                                            class="bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-bold"
                                                        >
                                                            <th
                                                                class="p-2 border border-slate-200 text-left"
                                                            >
                                                                Related Guidance
                                                            </th>
                                                            <th
                        ...
                        <!-- POLICIES RENDERER -->
                        <div
                            v-else-if="activeComponent === 'policies'"
                            class="space-y-4"
                        >
                            <div class="overflow-x-auto">
                                <table
                                ...
                                >
                                    ...
                                    >
                                        <tr>
                                            <th
                                                class="p-2 border border-slate-200 w-[90px]"
                                            >
                                                GAMO
                                            </th>
                                            <th
                                                class="p-2 border border-slate-200 w-[200px] text-left"
                                            >
                                                Policy
                                            </th>
                                            <th
                                            ...
                                            <th
                                                class="p-2 border border-slate-200 text-left w-[200px]"
                                            >
                                                Reference
                                            </th>
                                        </tr>
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="pol in activeObjectivePolicies"
                                            :key="pol.policy_id"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                                                class="p-2 border border-slate-200 text-center font-bold text-[#0f2b5c] dark:text-sky-400"
                                            >
                                                {{ activeObjectiveId }}
                                            </td>
                                            <td
                                                class="p-2 border border-slate-200 font-bold text-slate-700 dark:text-zinc-200 leading-normal"
                                            >
                                                {{ cleanText(pol.policy) }}
                                            </td>
                                            <td
                                                class="p-2 border border-slate-200 leading-relaxed"
                                            >
                                                {{ cleanText(pol.description) }}
                                            </td>
                                            <td
                                                class="p-2 border border-slate-200 leading-relaxed text-slate-655 dark:text-zinc-350"
                                                v-html="
                                                    cleanText(pol.guidance) ||
                                                    '-'
                                                "
                                            ></td>
                                            <td
                                                class="p-2 border border-slate-200 leading-relaxed text-slate-500 dark:text-zinc-400"
                                                v-html="
                                                    cleanText(pol.reference) ||
                                                    '-'
                                                "
                                            ></td>
                                        </tr>
                                        <tr
                                            v-if="
                                                !activeObjectivePolicies.length
                                            "
                                        >
                                            <td
                                                colspan="5"
                        ...
                        >
                            <div class="overflow-x-auto">
                                ...
                                >
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="sk in activeObjectiveSkills"
                                            :key="sk.skill_id"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                                            ...
                                            <td
                                                class="p-2 border border-slate-200 leading-relaxed text-slate-500 dark:text-zinc-400"
                                                v-html="
                                                    cleanText(sk.reference) ||
                                                    '-'
                                                "
                                            ></td>
                                        </tr>
                                        <tr
                                            v-if="!activeObjectiveSkills.length"
                                        >
                                            <td
                                                colspan="4"
                                                class="p-3 text-center text-slate-400 italic bg-white dark:bg-zinc-900"
                        ...
                        <div
                            v-else-if="activeComponent === 'culture'"
                            class="space-y-4"
                        >
                            <div class="overflow-x-auto">
                                <table
                                ...
                                >
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="cul in activeObjectiveCulture"
                                            :key="cul.keyculture_id"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                                                class="p-2 border border-slate-200 text-center font-bold text-[#0f2b5c] dark:text-sky-400"
                                            ...
                                            <td
                                                class="p-2 border border-slate-200 font-bold text-slate-700 dark:text-zinc-200 leading-normal"
                                            >
                                                {{ cleanText(cul.element) }}
                                            </td>
                                            <td
                                                class="p-2 border border-slate-200 leading-relaxed text-slate-655 dark:text-zinc-350"
                                                v-html="
                                                    cleanText(cul.guidance) ||
                                                    '-'
                                                "
                                            ></td>
                                            <td
                        ...
                        <!-- SERVICES RENDERER -->
                        <div
                            v-else-if="activeComponent === 'services'"
                            class="space-y-4"
                        >
                            <div class="overflow-x-auto">
                                <table
                                    ...
                                    >
                                        <tr>
                                            ...
                                            <th
                                                class="p-2 border border-slate-200 text-left"
                                            >
                                                Service / SIA Description
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr
                                            v-for="sia in activeObjectiveServices"
                                            :key="sia.sia_id"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                                            ...
                                            <td
                                                class="p-2 border border-slate-200 leading-normal"
                                            >
                                                {{ cleanText(sia.description) }}
                                            </td>
                                        </tr>
                                        <tr
                                            v-if="
                                                !activeObjectiveServices.length
                                            "
                                        >
                                            <td
                                                colspan="2"
                                                class="p-3 text-center text-slate-400 italic bg-white dark:bg-zinc-900"
                                            >
                                                No services / SIA found for this
                                                objective
                                            </td>
                                        </tr>
                                    </tbody>
                ...
                <div v-else-if="activeMode === 'component'" class="space-y-4">
                    ...
                    <div
                        class="bg-white dark:bg-zinc-900 p-3.5 border border-slate-200 dark:border-zinc-800 flex flex-col md:flex-row items-center gap-3"
                    >
                        <label
                            for="componentSelect"
                            class="text-xs font-bold text-slate-700 dark:text-zinc-300 w-24"
                            >Component</label
                        >
                        <select
                            id="componentSelect"
                        ...
                        >
                            <option value="">
                                -- Lihat per Component (semua objective) --
                            </option>
                            ...
                            <option value="services">
                                G.Component: Services, Infrastructure and
                                Applications
                            </option>
                        </select>
                    </div>
                    ...
                    <div v-if="selectedComponent" class="space-y-4">
                        ...
                        <div
                            class="bg-white dark:bg-zinc-900 p-2.5 border border-slate-200 dark:border-zinc-800 space-y-3"
                        >
                            <div
                                class="flex flex-wrap gap-1 border-b border-slate-100 dark:border-zinc-800 pb-2"
                            >
                        ...
                        <div
                            class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-sm p-4"
                        >
                            <h2
                                class="text-xs font-bold uppercase tracking-wider text-[#0f2b5c] dark:text-sky-400 mb-4 border-b border-slate-100 dark:border-zinc-800 pb-2"
                            >
                                Menampilkan
                                {{ COMPONENT_LABELS[selectedComponent] }} dari
                                {{
                                    componentObjectiveFilter === "ALL"
                                        ? "Semua Objective"
                                        : componentObjectiveFilter
                            ...
                            <div class="space-y-6">
                                <!-- Loop each objective -->
                                <div
                                    v-for="obj in filteredObjectivesForComponent"
                                    :key="obj.objective_id"
                                    class="border border-slate-250 dark:border-zinc-800 p-3 space-y-4"
                                >
                                    <div
                                        class="bg-[#0f2b5c] text-white px-3 py-1.5 font-bold text-xs"
                                    >
                                        {{ obj.objective_id }} —
                                        {{ cleanText(obj.objective) }}
                                    </div>

                                    <!-- Render overview inside loop -->
                                    <div
                                        v-if="selectedComponent === 'overview'"
                                        class="space-y-3"
                                    >
                                        <div
                                        ...
                                        >
                                            <div
                                                class="bg-slate-50 dark:bg-zinc-850 p-2 border border-slate-100 dark:border-zinc-800"
                                            >
                                                <div
                                                    class="font-bold text-slate-800 dark:text-zinc-200 mb-1"
                                                >
                                                    Description
                                                </div>
                                                <p>
                                    ...
                                    <div
                                        v-else-if="
                                            selectedComponent === 'practices'
                                        "
                                        class="space-y-4"
                                    >
                                        <div
                                            v-for="p in getPracticesForObjective(
                                                obj.objective_id,
                                            )"
                                            :key="p.practice_id"
                                            class="border border-slate-100 dark:border-zinc-800 text-xs"
                                        >
                                            <div
                                                class="bg-slate-50 dark:bg-zinc-800 px-2 py-1 font-bold text-[#0f2b5c] dark:text-sky-400 border-b border-slate-100 dark:border-zinc-700"
                                            >
                                                {{ p.practice_id }}
                                                {{ cleanText(p.practice_name) }}
                                            </div>
                                            <div
                                                class="p-2 text-slate-600 dark:text-zinc-300 leading-normal"
                                            >
                                                {{
                                                    cleanText(
                                                        p.practice_description,
                                    ...
                                    <div
                                        v-else-if="
                                            selectedComponent === 'infoflows'
                                        "
                                        class="space-y-2"
                                    >
                                        <table
                                            class="w-full border-collapse border border-slate-200 dark:border-zinc-800 text-xs"
                                        >
                                            <thead>
                                                <tr
                                                    class="bg-slate-50 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-bold border-b border-slate-200 dark:border-zinc-800"
                                                >
                                                    <th
                                                        class="p-1 border border-slate-200"
                                                    >
                                            ...
                                            <tbody>
                                                <tr
                                                    v-for="(
                                                        row, rIdx
                                                    ) in getInfoflowRowsForObjective(
                                                        obj.objective_id,
                                                    )"
                                                    :key="rIdx"
                                                    class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                                >
                                                    <td
                                                        v-if="
                                                            getInfoflowRowspansForObjective(
                                                                obj.objective_id,
                                                            )[rIdx] > 0
                                                        "
                                                        :rowspan="
                                                        ...
                                                        "
                                                        class="p-1 border border-slate-200 font-bold align-middle bg-slate-50/50 dark:bg-zinc-850/50"
                                                    >
                                                        {{ row.practiceId }}
                                                    </td>
                                                    <td
                                                    ...
                                                    <td
                                                        class="p-1 border border-slate-200 leading-relaxed"
                                                    >
                                                        {{
                                                            cleanText(row.input)
                                                        }}
                                                ...
                                                >
                                                    <td
                                                        colspan="5"
                                                        class="p-2 text-center text-slate-400 italic"
                                                    >
                                                        No information flows
                                                        found
                                                    </td>
                                    ...
                                    >
                                        <table
                                            class="w-full border-collapse border border-slate-200 dark:border-zinc-800 text-[11px]"
                                        >
                                            <thead>
                                                <tr
                                                    class="bg-[#d2e2d8] dark:bg-zinc-800 text-slate-800 dark:text-zinc-200"
                                                >
                                                    <th
                                                        class="p-1.5 border border-slate-200 text-left min-w-[260px] align-middle"
                                                    >
                                                        MANAGEMENT PRACTICE OF
                                                        GOVERNANCE AND
                                                    ...
                                                    <th
                                                        v-for="rn in getOrgRolesForObjective(
                                                            obj.objective_id,
                                                        ).roles"
                                                        :key="rn"
                                                        class="p-0 border border-slate-200 w-[44px] text-center align-middle"
                                                    >
                                                        <div
                                                            class="vertical-text text-[10px] font-bold py-1 select-none text-slate-855 dark:text-zinc-250 uppercase"
                                                        >
                                                            {{ cleanText(rn) }}
                                                        </div>
                                                    </th>
                                            ...
                                            <tbody>
                                                <tr
                                                    v-for="p in getOrgRolesForObjective(
                                                        obj.objective_id,
                                                    ).practices"
                                                    :key="p.practice_id"
                                                    class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                                >
                                                    <td
                                                        class="p-1.5 border border-slate-200 font-bold text-slate-800 dark:text-zinc-300"
                                                    >
                                                        {{ p.practice_id }}
                                                        {{
                                                            cleanText(
                                                    ...
                                                    <td
                                                        ...
                                                        :key="rn"
                                                        class="p-1.5 border border-slate-200 text-center font-extrabold text-sm"
                                                        :class="
                                                            getRaciColorClass(
                                                                p
                                                                    .roleAssignments[
                                    ...
                                    >
                                        ...
                                        >
                                            <thead>
                                                <tr
                                                    class="bg-slate-50 dark:bg-zinc-800"
                                                >
                                                    <th
                                                        class="p-1 border border-slate-200 text-left w-[180px]"
                                                    >
                                                        Policy
                                                    </th>
                                                    <th
                                                        class="p-1 border border-slate-200 text-left"
                                                    >
                                                        Description
                                                    </th>
                                                    <th
                                                        class="p-1 border border-slate-200 text-left w-[200px]"
                                                    >
                                                        Reference
                                                    </th>
                                                </tr>
                                            ...
                                            <tbody>
                                                <tr
                                                    v-for="p in getPoliciesForObjective(
                                                        obj.objective_id,
                                                    )"
                                                    :key="p.policy_id"
                                                >
                                                    <td
                                                        class="p-1.5 border border-slate-200 font-bold"
                                                    >
                                                        {{
                                                            cleanText(p.policy)
                                                        }}
                                                    ...
                                                    <td
                                                        class="p-1.5 border border-slate-200 leading-normal"
                                                    >
                                                        {{
                                                            cleanText(
                                                                p.description,
                                                    ...
                                                    <td
                                                        class="p-1.5 border border-slate-200 text-slate-550"
                                                        v-html="
                                                            cleanText(
                                                                p.reference,
                                                            ) || '-'
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <tbody>
                                                <tr
                                                    v-for="s in getSkillsForObjective(
                                                        obj.objective_id,
                                                    )"
                                                    :key="s.skill_id"
                                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <tbody>
                                                <tr
                                                    v-for="c in getCultureForObjective(
                                                        obj.objective_id,
                                                    )"
                                                    :key="c.keyculture_id"
                                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <tbody>
                                                <tr
                                                    v-for="s in getServicesForObjective(
                                                        obj.objective_id,
                                                    )"
                                                    :key="s.sia_id"
                                                >
                ...
                <div v-else-if="activeMode === 'master'" class="space-y-4">
                    ...
                    >
                        <h2
                            class="text-sm font-bold uppercase tracking-wider text-[#0f2b5c] dark:text-sky-400 border-b border-slate-100 dark:border-zinc-800 pb-2 mb-4"
                        >
                            Data Master
                        </h2>
                        ...
                        <!-- Master Tab Buttons -->
                        <div
                            class="flex border-b border-slate-200 dark:border-zinc-800 mb-4"
                        >
                            <button
                        ...
                        >
                            <div class="overflow-x-auto">
                                ...
                                >
                                    <thead
                                        class="bg-[#0f2b5c] text-white font-bold text-left"
                                    >
                                        <tr>
                                            <th
                                                class="p-2 border border-slate-200 w-[140px]"
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="eg in masterGoalsData.entergoals"
                                            :key="eg.id"
                                            class="hover:bg-slate-50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                                                class="p-2 border border-slate-200 font-bold text-[#0f2b5c] dark:text-sky-400"
                                            >
                                                {{ eg.id }}
                                            </td>
                                            <td
                                                class="p-2 border border-slate-200 text-slate-700 dark:text-zinc-300 leading-normal"
                                            >
                                                {{ cleanText(eg.description) }}
                                            </td>
                                        </tr>
                        ...
                        >
                            <div class="overflow-x-auto">
                                ...
                                >
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="ag in masterGoalsData.aligngoals"
                                            :key="ag.id"
                                            class="hover:bg-slate-50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                        ...
                        <!-- Panel: Roles -->
                        <div
                            v-show="activeMasterTab === 'roles'"
                            class="space-y-4"
                        >
                            <div
                                class="text-[11px] text-slate-500 italic flex items-center gap-1.5 bg-slate-50 dark:bg-zinc-800 p-2 border-l-4 border-sky-400"
                            >
                                <span
                                    class="font-bold text-sky-600 dark:text-sky-400"
                                    >💡 Tip:</span
                                >
                                Klik pada baris role untuk melihat pemetaan GAMO
                                mana saja yang didukung role tersebut beserta
                                RACI-nya.
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                ...
                                >
                                    ...
                                    >
                                        <tr>
                                            <th
                                                class="p-2 border border-slate-200 w-[180px]"
                                            >
                                                Role Name
                                            </th>
                                            <th
                                    ...
                                    <tbody>
                                        <tr
                                            ...
                                            :class="
                                                ...
                                                r.name
                                                    ? 'bg-indigo-50/50 dark:bg-indigo-950/20 border-l-4 border-[#0f2b5c] font-semibold'
                                                    : ''
                                            "
                                        >
                                            <td
                                                class="p-2 border border-slate-200 font-bold text-slate-700 dark:text-zinc-200"
                                            >
                                                {{ cleanText(r.name) }}
                                            </td>
                                            <td
                                                class="p-2 border border-slate-200 text-slate-500 dark:text-zinc-400"
                                            >
                                                {{
                            ...
                            <div
                                ...
                                class="mt-4 border border-sky-300 dark:border-zinc-700 bg-sky-50/10 dark:bg-zinc-900 shadow-sm"
                            >
                                <div
                                    class="bg-sky-600 text-white p-3.5 font-bold text-xs flex justify-between items-center"
                                >
                                    <span
                                        >ROLE DETAIL MATRIX:
                                        {{
                                            cleanText(selectedRoleDetail.name)
                                        }}
                                        (Muncul di
                                        {{
                                            activeRoleGamoRows.length
                                        }}
                                        Practice)</span
                                    >
                                    <button
                                        @click="selectedRoleDetail = null"
                                        class="text-white hover:text-sky-200 text-sm font-extrabold"
                                    >
                                        &times;
                                    </button>
                                </div>
                                <div class="overflow-x-auto">
                                    ...
                                    >
                                        <thead
                                            class="bg-slate-50 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-bold"
                                        >
                                            <tr>
                                                <th
                                                    class="p-2 border border-slate-200 text-center w-[120px]"
                                                >
                                                    GAMO
                                                </th>
                                                <th
                                                ...
                                                <th
                                                    class="p-2 border border-slate-200 text-center w-[100px]"
                                                >
                                                    RACI
                                                </th>
                                            </tr>
                                        ...
                                        <tbody>
                                            <tr
                                                v-for="d in activeRoleGamoRows"
                                                :key="d.practice_id"
                                                class="hover:bg-slate-100 dark:hover:bg-zinc-850"
                                            >
                                                <td
                                                    class="p-2 border border-slate-200 text-center font-bold text-[#0f2b5c] dark:text-sky-400"
                                                >
                                                ...
                                                <td
                                                    class="p-2 border border-slate-200 text-center font-extrabold text-[#7a2433] dark:text-rose-450"
                                                >
                                                    {{ d.raci || "-" }}
                                                </td>
                                            </tr>
                            ...
                            <div class="overflow-x-auto">
                                ...
                                >
                                    ...
                                    >
                                        <tr>
                                            <th
                                                class="p-2 border border-slate-200 w-[100px]"
                                            >
                                                GAMO
                                            </th>
                                            <th
                                                class="p-2 border border-slate-200 w-[240px] text-left"
                                            >
                                                Practice
                                            </th>
                                            <th
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="(
                                                row, idx
                                            ) in filteredCapabilityRows"
                                            :key="idx"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-850"
                                        >
                                            <td
                                                class="p-2 border border-slate-200 text-center font-bold text-slate-700 dark:text-zinc-300"
                                            >
                                                {{ row.gamo }}
                                            </td>
                                            <td
                                            ...
                                            >
                                                {{ row.practice_id }} —
                                                {{
                                                    cleanText(row.practice_name)
                                                }}
                                            </td>
                                            <td
                                                class="p-2 border border-slate-200 leading-relaxed text-slate-655 dark:text-zinc-400"
                                            >
                                            ...
                                            <td
                                                v-if="
                                                    capabilityRowspans[idx] > 0
                                                "
                                                :rowspan="
                                                    capabilityRowspans[idx]
                                                "
...
async function loadComponentData(compName) {
    ...
    try {
        ...
        if (result.success) {
            ...
            cachedComponents.value[compName] = result.data.filter(
                (obj) => obj.objective_id && !obj.objective_id.includes("."),
            );
        } else {
...
async function loadGamoData(focusAreaId = null) {
    ...
    try {
        ...
        if (result.success) {
            ...
            gamoObjectives.value = result.objectives.filter(
                (obj) => obj.objective_id && !obj.objective_id.includes("."),
            );
    ...
    try {
        ...
        if (result.success) {
            ...
            if (focusAreas.value.length) {
                const coreModel = focusAreas.value.find(
                    (fa) => fa.code === "CORE MODEL" || fa.id === 1,
                );
...
async function selectFocusArea(focusArea) {
    ...
    if (gamoObjectives.value.length) {
        const exists = gamoObjectives.value.some(
            (o) => o.objective_id === activeObjectiveId.value,
        );
        if (!exists) {
    ...
    async () => {
        if (activeMode.value === "gamo") {
            if (activeComponent.value === "overview") {
                await Promise.all([
                    loadComponentData("overview"),
                    loadComponentData("goals"),
                ]);
...
function selectDomain(dom) {
    ...
    const filtered = gamoObjectives.value.filter((o) =>
        o.objective_id.startsWith(dom),
    );
    if (filtered.length) {
...
const activeObjectiveName = computed(() => {
    const obj = gamoObjectives.value.find(
        (o) => o.objective_id === activeObjectiveId.value,
    );
...
const activeDomainName = computed(() => {
    return cleanText(
        domainNamesMap[activeDomain.value] || "IT Governance Domain",
    );
});

// Objectives list filtered for Gamo mode
const filteredObjectives = computed(() => {
    return gamoObjectives.value.filter((o) =>
        o.objective_id.startsWith(activeDomain.value),
    );
});
...
const activeObjectivePractices = computed(() => {
    ...
    return obj.practices.map((p) => {
        ...
        const rowspans = computeRowspans(p.activities || [], (a) =>
            cleanText(String(a.capability_lvl ?? a.capability_level ?? "-")),
        );
        return {
            ...p,
...
function getInfoflowRows(objId) {
    ...
    const practicesList =
        cachedComponents.value.practices?.find((o) => o.objective_id === objId)
            ?.practices || [];
    const practicesMap = {};
...
const activeObjectiveInfoflowRowspans = computed(() => {
    return computeRowspans(
        activeObjectiveInfoflowRows.value,
        (r) => r.practiceId,
    );
});
...
const activeObjectiveOrganizational = computed(() => {
    ...
    obj.practices.forEach((p) => {
        (p.roles || []).forEach((r) => {
            if (r.role_name || r.role)
                rolesSet.add(cleanText(r.role_name || r.role));
        });
    });
...
const activeObjectivePolicies = computed(() => {
    ...
    return obj.policies.map((p) => {
        ...
        return {
            ...
            guidance: guidances
                .map((g) => cleanText(g.guidance))
                .filter(Boolean)
                .join("<br>"),
            reference: guidances
                .map((g) => cleanText(g.reference))
                .filter(Boolean)
                .join("<br>"),
        };
    });
...
const filteredObjectivesForComponentMode = computed(() => {
    ...
    return gamoObjectives.value.filter((o) =>
        o.objective_id.startsWith(componentDomainFilter.value),
    );
});
...
const filteredObjectivesForComponent = computed(() => {
    ...
    if (componentObjectiveFilter.value !== "ALL") {
        list = list.filter(
            (o) => o.objective_id === componentObjectiveFilter.value,
        );
    }
    return list;
});
...
function getInfoflowRowspansForObjective(objId) {
    return computeRowspans(
        getInfoflowRowsForObjective(objId),
        (r) => r.practiceId,
    );
}
...
function getPoliciesForObjective(objId) {
    const obj = cachedComponents.value.policies?.find(
...
function getSkillsForObjective(objId) {
    const obj = cachedComponents.value.skills?.find(
        (o) => o.objective_id === objId,
    );
    if (!obj || !obj.skills) return [];
    return obj.skills.map((s) => ({
...
function getCultureForObjective(objId) {
    const obj = cachedComponents.value.culture?.find(
        (o) => o.objective_id === objId,
    );
    if (!obj || !obj.culture) return [];
    return obj.culture.map((c) => ({
...
function getServicesForObjective(objId) {
    const list =
        cachedComponents.value.services?.find((o) => o.objective_id === objId)
            ?.s_i_a || [];
    return list.map((s) => ({
        ...s,
...
const masterGoalsData = computed(() => {
    ...
    goalsComp.forEach((obj) => {
        (obj.entergoals || []).forEach((eg) => {
            const id = String(eg.entergoals_id || "")
                .toUpperCase()
                .trim();
            if (id && !egMap.has(id)) {
        ...
        (obj.aligngoals || []).forEach((ag) => {
            const id = String(ag.aligngoals_id || "")
                .toUpperCase()
                .trim();
            if (id && !agMap.has(id)) {
    ...
    const entergoals = Array.from(egMap.values()).sort((a, b) =>
        a.id.localeCompare(b.id),
    );
    const aligngoals = Array.from(agMap.values()).sort((a, b) =>
        a.id.localeCompare(b.id),
    );
