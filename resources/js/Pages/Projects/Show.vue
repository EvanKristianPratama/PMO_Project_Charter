<template>
    <div class="min-h-screen bg-slate-100 print:bg-white">
        ...
        <nav class="bg-white border-b border-slate-200 sticky top-0 z-30 print:hidden">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center gap-4">
                        <Link href="/projects" class="text-slate-500 hover:text-indigo-600">
                            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                            </svg>
                        </Link>
                        <h1 class="text-lg font-bold text-slate-900 truncate">{{ project.name }} <span class="text-slate-400 font-normal">({{ project.code }})</span></h1>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button
                            @click="printCharter"
                            class="p-2 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 rounded-lg"
                            title="Print PDF"
                        >
