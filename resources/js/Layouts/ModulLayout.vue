...
const isItspsSubmenuActive = computed(() => {
    return navItems.value.some(item => 
        itspsLabels.includes(item.label) && typeof item.active === 'function' && item.active(currentUrl.value)
    );
});

const isItomSubmenuActive = computed(() => {
    return navItems.value.some(item => 
        itomLabels.includes(item.label) && typeof item.active === 'function' && item.active(currentUrl.value)
    );
});
    ...
    <div class="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0f0f0f] dark:text-slate-100">
                ...
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-3">
                        <div class="hidden md:block">
                            <Link :href="route('dashboard')" class="group inline-flex items-center gap-2.5 mb-1">
                                <img
                                    src="/CISS1.png"
                                    alt="Logo"
                                ...
                                />
                                <div class="hidden md:block">
                                    <p class="text-sm font-semibold tracking-tight text-slate-900 dark:text-white leading-none">Collaboration Information System</p>
                                    <div class="flex items-center gap-1.5 mt-1">
                                <Link
                                    v-if="activeModul === 'all' || activeModul === 'itsps'"
                                    ...
                                    :class="[
                                        activeModul === 'all' && isItomSubmenuActive
                                ...
                                <Link
                                    v-if="activeModul === 'all' || activeModul === 'itom'"
                                    :href="route('business-process.proses-bisnis.index')"
                                    class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-[9.5px] font-extrabold uppercase tracking-wider shadow-sm select-none transition-all duration-205"
                                    :class="[
                                        activeModul === 'all' && isItspsSubmenuActive
                                            ? 'bg-slate-100 text-slate-400 border-slate-200 hover:bg-slate-200 hover:text-slate-600 dark:bg-zinc-800/60 dark:text-zinc-500 dark:border-zinc-800 dark:hover:bg-zinc-700/60 dark:hover:text-zinc-300'
        ...
        <div class="sticky top-16 z-40 print:hidden" v-if="activeModul === 'all' || activeModul === 'itsps' || activeModul === 'itom'">
            <div class="flex flex-col gap-2 border-b border-white/50 bg-white/40 px-4 py-2 backdrop-blur-md dark:border-white/10 dark:bg-white/5 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
                ...
                <div v-if="activeModul === 'itsps' || (activeModul === 'all' && (!isItomSubmenuActive || isItspsSubmenuActive))" class="w-full min-w-0 rounded-2xl border border-white/70 bg-white/60 px-3 py-1.5 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-white/5 sm:w-auto">
                ...
                <div v-if="activeModul === 'itom' || (activeModul === 'all' && (!isItspsSubmenuActive || isItomSubmenuActive))" class="w-full min-w-0 rounded-2xl border border-white/70 bg-white/60 px-3 py-1.5 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-white/5 sm:w-auto">
                    <div class="overflow-x-auto" :class="{ 'sm:flex sm:justify-end': activeModul === 'all' && !isItspsSubmenuActive && !isItomSubmenuActive }">
                        <div class="min-w-max">
                            <BreadcrumbRight />
                        </div>
                    </div>
        ...
        <footer class="mx-auto w-full max-w-7xl px-4 pb-8 sm:px-6 lg:px-8 print:hidden">
            <div class="border-t border-slate-200/70 pt-5 dark:border-white/5">
                <p class="text-center text-xs text-slate-400 dark:text-slate-500">Collaboration System</p>
            </div>
        </footer>
    </div>
</template>
