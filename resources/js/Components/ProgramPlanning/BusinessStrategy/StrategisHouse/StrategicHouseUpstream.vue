<template>
    <div class="flex flex-col items-center w-full max-w-6xl mx-auto space-y-1 p-4">
        ...
        <div class="relative w-[90%] flex justify-center mt-4" v-if="strategicHouseUpstream">
            <div class="w-full bg-[#1c2a5e] text-white pt-12 pb-4 px-12 text-center" style="clip-path: polygon(50% 0%, 100% 100%, 0% 100%); min-height: 140px; display: flex; flex-direction: column; justify-content: flex-end;">
                <div class="font-bold text-xs italic mb-1">Vision:</div>
                <div class="font-semibold text-sm max-w-xl mx-auto leading-tight">{{ strategicHouseUpstream.vision }}</div>
            </div>
        </div>
        ...
        <template v-if="upstreamGoals && upstreamGoals.length > 0">
            ...
            <div class="w-[90%] flex gap-2 mt-0">
                <div v-for="goal in getMainGoals(upstreamGoals)" :key="'pillar-' + goal.id"
                    ...
                    <div class="w-full h-3 bg-gradient-to-b from-[#0e4d7a] to-[#1866a5] rounded-t-sm shadow-md"></div>

                    <!-- Theme Pillar Columns -->
                    <div class="w-full flex items-stretch"
                        :style="{ minHeight: maxPillarHeight + 'px' }">
                        <template v-for="(theme, index) in getPillarThemes(goal.themes)" :key="theme.id">
                            <div class="flex-1 flex flex-col">
                                ...
                                <div class="relative flex-1 mx-1 flex flex-col pillar-column">
                                    ...
                                    <!-- Theme Header -->
                                    <div class="relative z-10 border-b border-white/30 py-3 px-2 text-center bg-[#1256a0]/80">
                                        <h3 class="font-bold text-xs italic text-white drop-shadow-sm">{{ theme.name }}</h3>
                                    </div>
                                    ...
                                    <div class="relative z-10 p-2 flex-1 text-[10px]">
                                        <ul class="space-y-2 list-none pl-0">
                                            <li v-for="pillar in theme.pillar_themes" :key="pillar.id"
                                                ...
                                                <span class="absolute left-0 top-1.5 w-1 h-1 rounded-full bg-yellow-300 shadow-sm shadow-yellow-300/50"></span>
                                                <span class="italic text-yellow-300 font-semibold">{{ pillar.title }}</span>
                    ...
                    <div class="w-full h-3 bg-gradient-to-b from-[#0a3a5e] to-[#071e30] shadow-lg rounded-b-sm"></div>

                    <!-- Enabler Theme (from individual goal if exists) -->
                    <div v-if="getEnablerTheme(goal.themes)"
                        class="w-full bg-[#00a688] text-white text-center py-1.5 px-4 shadow-sm mt-1">
                        <h3 class="font-bold text-xs italic">{{ getEnablerTheme(goal.themes).name }}</h3>
                    </div>
                </div>
            </div>
...
const getPillarThemes = (themes) => {
    if (!themes) return [];
    return themes.filter(t => t.theme_number !== 20);
};

const getEnablerTheme = (themes) => {
    if (!themes) return null;
    return themes.find(t => t.theme_number === 20);
};
