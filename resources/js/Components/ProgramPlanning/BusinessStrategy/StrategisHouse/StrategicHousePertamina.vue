    ...
    <div class="flex flex-col items-center w-full max-w-6xl mx-auto space-y-1 p-4">
        ...
        <div class="relative w-[90%] flex justify-center mt-4" v-if="strategicHousePertamina">
            <div class="w-full bg-[#1c2a5e] text-white pt-12 pb-4 px-12 text-center" style="clip-path: polygon(50% 0%, 100% 100%, 0% 100%); min-height: 140px; display: flex; flex-direction: column; justify-content: flex-end;">
                ...
                <div class="font-semibold text-sm max-w-xl mx-auto leading-tight">{{ strategicHousePertamina.vision }}</div>
            </div>
        </div>
        ...
        <template v-if="pertaminaGoals && pertaminaGoals.length > 0">
            <div v-for="goal in pertaminaGoals" :key="goal.id" class="w-full flex flex-col items-center">
                        ...
                        <div class="flex-1 bg-[#1866a5] text-white flex flex-col shadow-sm border border-blue-400/20">
                            ...
                            <div class="p-4 flex-1 text-xs">
                                <ul class="space-y-3 list-none pl-1">
                                    <li v-for="pillar in theme.pillar_themes" :key="pillar.id" class="relative pl-4 leading-relaxed">
                ...
                <div v-if="getEnablerTheme(goal.themes)" class="w-[90%] bg-[#00a688] text-white text-center py-2 px-6 shadow-sm mt-2">
