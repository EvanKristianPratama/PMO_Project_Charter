<template>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="rounded-2xl border border-slate-900 bg-white p-5 dark:border-white/20 dark:bg-[#171717]">
            <div class="border-b border-slate-100 pb-3 mb-4 dark:border-white/5 flex items-center justify-between">
                <h3 class="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">
                    Skor Kelengkapan Dokumen Master
                </h3>
        ...
        <div class="rounded-2xl border border-slate-900 bg-white p-5 dark:border-white/20 dark:bg-[#171717]">
            <div class="border-b border-slate-100 pb-3 mb-4 dark:border-white/5 flex items-center justify-between">
                <h3 class="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">
                    Penyebab Ketidaklengkapan Dokumen Master Digital Initiative
                </h3>
...
const chartData = computed(() => {
    ...
    props.projects.forEach(project => {
        if (!project.details || !project.details.master) return;
        
        Object.keys(fieldLabels).forEach(field => {
            if (!project.details.master[field]) {
...
// Group projects by completeness score of Master document
const scoreDistribution = computed(() => {
    const counts = {};
    props.projects.forEach(project => {
        const score = project.master_score || '0%';
...
const updateScoreBarChart = () => {
    if (!scoreBarChartCanvas.value || props.projects.length === 0) {
