...
const submitForm = () => {
    ...
    if (editingMilestoneId.value) {
        milestoneForm.put(route('itsp.it-initiatives.milestones.update', [charterId, editingMilestoneId.value]), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => resetForm(),
        });
    } else {
        milestoneForm.post(route('itsp.it-initiatives.milestones.store', charterId), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => resetForm(),
        });
...
const removeMilestone = (milestoneId) => {
    ...
    router.delete(route('itsp.it-initiatives.milestones.destroy', [charterId, milestoneId]), {
        preserveScroll: true,
        preserveState: true,
    });
};
