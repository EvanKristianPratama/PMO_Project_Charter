...
const saveEditMode = () => {
    ...
    router.put(route('itsp.strategic-house.business-strategy.bulk-update'), {
...
const deleteStrategy = (row) => {
    ...
    Swal.fire({
        ...
        router.delete(route('itsp.strategic-house.business-strategy.destroy', row.id), {
            preserveScroll: true,
            preserveState: true,
            onFinish: () => {
                strategyProcessing.value = false;
...
const submitStrategy = () => {
    ...
    router.post(
        route('itsp.strategic-house.business-strategy.store'),
        buildPayload(),
        requestOptions,
    );
};
