                    ...
                    <tr v-for="(item, index) in filteredItems" :key="`mst-digital-${item.id}`" class="transition-colors hover:bg-slate-50 dark:hover:bg-white/5">
                        ...
                        <td class="px-3 py-3 text-[10px] font-medium">
                            <div class="flex flex-col items-start gap-1">
                                ...
                                <Link
                                    :href="item.mappedProjects?.[0]?.id ? route('itsp.it-initiatives.show', { project: item.mappedProjects[0].id, tab: 'detail' }) : route('itsp.it-initiatives.index', { tableMode: 'implementation' })"
...
const initiativeHref = (item) => {
    ...
    return matchedInitiative?.id ? route('itsp.digital-initiatives.show', matchedInitiative.id) : route('itsp.digital-initiatives.index');
};
