...
const submitForm = () => {
    ...
    if (modalMode.value === 'create') {
        form.transform(() => payload).post(route('itom.business-process.proses-bisnis-v2.store'), {
            onSuccess: () => {
        ...
        form.transform(() => payload).put(route('itom.business-process.proses-bisnis-v2.update', selectedItem.value.id), {
            onSuccess: () => {
                isModalOpen.value = false;
                form.reset();
            },
...
const submitDelete = () => {
    form.delete(route('itom.business-process.proses-bisnis-v2.destroy', selectedItem.value.id), {
        onSuccess: () => {
