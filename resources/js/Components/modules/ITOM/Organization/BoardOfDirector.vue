...
const bodsOnly = computed(() => {
    return props.bods.filter(bod => bod.tipe && String(bod.tipe).toLowerCase() === 'bod');
});

const filteredBods = computed(() => {
    if (!selectedCompanyId.value) {
        return bodsOnly.value;
    }
...
const availableParents = computed(() => {
    return bodsOnly.value.filter(bod => {
...
const getParentName = (parentId) => {
    ...
    const parent = bodsOnly.value.find(b => b.id === Number(parentId));
...
const submitBODForm = () => {
    if (bodModalMode.value === 'create') {
        bodForm.post(route('itom.business-process.organization-structure.bod.store'), {
            onSuccess: () => {
        ...
        bodForm.put(route('itom.business-process.organization-structure.bod.update', selectedBod.value.id), {
            onSuccess: () => {
                isBODModalOpen.value = false;
                bodForm.reset();
            },
...
const submitDeleteBOD = () => {
    bodForm.delete(route('itom.business-process.organization-structure.bod.destroy', selectedBod.value.id), {
        onSuccess: () => {
