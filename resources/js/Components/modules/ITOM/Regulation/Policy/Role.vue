<template>
    <div class="space-y-6 animate-fade-in-up print:m-0 print:p-0">
            ...
            <div v-show="activeSubMenu === 'document'" class="max-w-4xl mx-auto bg-white dark:bg-[#1a1a1a] shadow-xl border border-slate-200 dark:border-white/10 p-6 sm:p-8 md:p-10 rounded-2xl relative text-justify print:shadow-none print:border-none print:p-0 print:m-0"
                style="font-family: Arial, sans-serif; font-size: 11pt; line-height: 1.5; color: #1e293b;">
                
                <!-- Formal Pertamina Document Grid Header -->
                <PertaminaDocumentHeader v-if="isHeaderVisible" :activeRegulation="activeRegulation" class="mb-8" />
            ...
            <div v-show="activeSubMenu === 'policy_mapping'" class="w-full bg-white dark:bg-[#1a1a1a] shadow-xl border border-slate-200 dark:border-white/10 p-6 sm:p-8 md:p-10 rounded-2xl relative text-justify print:shadow-none print:border-none print:p-0 print:m-0 overflow-visible"
                ...
                <div class="text-center space-y-2 relative z-10 mb-6">
                    <h2 class="text-lg sm:text-xl font-extrabold tracking-[0.2em] text-slate-955 dark:text-white uppercase">
                    ...
                    <div class="print:hidden flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-200/60 dark:border-white/10 shadow-sm mb-6">
                        ...
                        <div v-if="!readonly" class="flex items-center gap-2 shrink-0">
                            ...
                            <template v-else>
                                ...
                                <button 
                                    @click="cancelEditingGlobal"
                                    class="inline-flex items-center gap-1.5 rounded-xl border border-slate-300 bg-white hover:bg-slate-50 dark:border-slate-300 dark:bg-[#1a1a1a] dark:hover:bg-white/5 px-4 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 transition active:scale-95 whitespace-nowrap"
                                >
                                    Batal
                                </button>
                            </template>
                        ...
                        <table class="min-w-full border-collapse text-left">
                            <thead>
                                <tr class="bg-[#d2e4df] dark:bg-[#1b3a32] text-slate-800 dark:text-slate-200 divide-x divide-[#b2cfc7] dark:divide-[#255246] border-b border-[#b2cfc7] dark:border-[#255246] text-xs uppercase tracking-wider font-bold">
                                    ...
                                    <th class="p-3.5 text-left min-w-[320px]">
                                        Kebijakan Khusus Terhubung (COBIT Objective)
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-200 dark:divide-white/10 text-slate-800 dark:text-slate-200">
                                <tr v-for="(resp, rIdx) in filteredAllResponsibilities" :key="resp.id" class="hover:bg-slate-50 dark:hover:bg-white/5 divide-x divide-slate-100 dark:divide-white/5 transition-colors print:hover:bg-transparent">
                                    <!-- Row number -->
                                    <td class="p-3.5 border-r border-slate-200 dark:border-white/10 text-center font-mono font-bold text-slate-500 dark:text-slate-400 text-xs bg-slate-50/40 dark:bg-white/5 select-none">
                                        {{ rIdx + 1 }}
                                    </td>
                                    ...
                                    <!-- Tanggung Jawab text -->
                                    <td class="p-3.5 border-r border-slate-200 dark:border-white/10 text-left font-serif text-[11px] leading-relaxed text-slate-900 dark:text-slate-100 text-justify align-top">
                                        {{ resp.responsible }}
                                    </td>
                                    ...
                                    <td class="p-3.5 align-top relative">
                                        ...
                                        <div v-if="!isEditing" class="flex flex-wrap gap-1.5">
                                            <span 
                                                v-for="obj in getMappedObjectivesList(resp.id)" 
                                                :key="obj.objective_id"
                                                class="inline-flex items-center rounded-lg bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 text-[10px] font-black text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/20"
                                                :title="obj.objective"
                                            >
                                                {{ obj.objective_id }}
                                            </span>
                                            <span v-if="getMappedObjectivesList(resp.id).length === 0" class="text-xs text-slate-400 dark:text-slate-500 italic">
                                                Belum terhubung ke kebijakan.
                                            </span>
                                        </div>
                                        ...
                                        <div v-else class="flex flex-col gap-2.5">
                                            <div class="relative">
                                                <button
                                                    @click.stop="toggleAddObjectiveDropdown(resp.id)"
                                                    class="w-full text-left inline-flex items-center justify-between gap-2 rounded-xl border border-slate-300 dark:border-white/10 bg-white hover:bg-slate-50 dark:bg-[#1a1a1a] dark:hover:bg-white/5 px-3 py-2.5 text-xs font-semibold text-slate-700 dark:text-slate-300 shadow-sm active:scale-95 transition-all"
                                                >
                                                    <span class="truncate font-semibold">
                                                        {{ getSelectedObjectivesCountText(resp.id) }}
                                                    </span>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.2" stroke="currentColor" class="w-4 h-4 text-slate-400 shrink-0">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                                                    </svg>
                                                ...
                                                <!-- Dropdown Menu for choosing objectives -->
                                                <div 
                                                    v-if="activeAddDropdownObjId === resp.id"
                                                    class="absolute right-0 mt-1.5 z-40 w-80 max-h-72 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-xl dark:border-white/10 dark:bg-[#222] p-2 divide-y divide-slate-100 dark:divide-white/5 space-y-2"
                                                >
                                                    <div class="px-2.5 py-1.5 flex items-center justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wider select-none">
                                                        <span>Hubungkan Kebijakan</span>
                                                        <span class="text-emerald-700 dark:text-emerald-300 font-extrabold">{{ getMappedObjectivesList(resp.id).length }} Terpilih</span>
                                                    </div>
                                                    ...
                                                    <div class="p-1">
                                                        <input 
                                                            ...
                                                            type="text"
                                                            placeholder="Cari kebijakan COBIT..."
                                                            class="w-full px-2.5 py-1 text-[11px] rounded-lg border border-slate-200 bg-white dark:border-white/10 dark:bg-[#1a1a1a] dark:text-white shadow-inner"
                                                            @click.stop
                                                        />
                                                    </div>
                                                    ...
                                                    <div class="py-1 max-h-48 overflow-y-auto space-y-0.5" @click.stop>
                                                        ...
                                                        >
                                                            ...
                                                            <div class="text-[11px] leading-tight">
                                                                <span class="font-bold text-emerald-800 dark:text-emerald-300 block mb-0.5">{{ obj.objective_id }}</span>
                                                                <span>{{ obj.objective }}</span>
                                                            </div>
                                                        </div>
                                                        <div v-if="filteredObjectivesForDropdown.length === 0" class="px-3 py-4 text-center text-xs text-slate-400 dark:text-slate-500">
                                            ...
                                            <!-- Visual badges with remove trigger (only while editing) -->
                                            <div class="flex flex-wrap gap-1.5">
                                                <div 
                                                    v-for="obj in getMappedObjectivesList(resp.id)" 
                                                    :key="obj.objective_id"
                                                    class="inline-flex items-center gap-1 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 text-[10px] font-black text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/20"
                                                    :title="obj.objective"
                                                >
                                                    <span>{{ obj.objective_id }}</span>
                                                    <button 
                                                        @click.stop="toggleMapping(resp.id, obj.objective_id)"
                                                        class="hover:text-rose-600 text-slate-400 focus:outline-none font-bold text-xs"
                                                        title="Hapus Pemetaan"
                                                    >
                                                        ×
                                                    </button>
                                ...
                                <tr v-if="filteredAllResponsibilities.length === 0">
                                    <td colspan="3" class="p-8 text-center text-slate-400 dark:text-slate-500 font-sans text-xs">
                                        Tidak ada data tanggung jawab untuk ditampilkan.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
