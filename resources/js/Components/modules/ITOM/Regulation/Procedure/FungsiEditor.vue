...
function submitActorForm() {
    ...
    if (editingActorId.value) {
        actorForm.put(route('itom.policy.regulation.procedure.actor.update', editingActorId.value), {
            preserveScroll: true,
            onSuccess: () => {
                closeActorModal();
                Swal.fire({
        ...
        actorForm.post(route('itom.policy.regulation.procedure.actor.store'), {
            preserveScroll: true,
            onSuccess: () => {
                closeActorModal();
                Swal.fire({
...
function deleteActor(actor) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('itom.policy.regulation.procedure.actor.destroy', actor.id), { preserveScroll: true });
        }
    });
}
